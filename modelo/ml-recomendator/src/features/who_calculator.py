...
class WHOCalculator:
    ...
    def __init__(self, who_dir: Path | str | None = None, db_connector=None):
        """
        Args:
    ...
    def _load_lms_from_csv(self) -> pd.DataFrame:
        ...
        return pd.concat(
            [self._load_lms_by_sex("M"), self._load_lms_by_sex("F")], ignore_index=True
        )
    ...
    def get_lms(self, age_months: int, sex: str) -> tuple[float, float, float]:
        ...
        # Buscar más cercano
        nearest = subset.iloc[(subset["month"] - age_months).abs().argsort().iloc[0]]
        return float(nearest["L"]), float(nearest["M"]), float(nearest["S"])

    def calculate_baz(self, bmi: float, age_months: int, sex: str) -> float:
        """
    ...
    def process_dataframe(
        ...
        # Calcular BAZ
        df["baz"] = df.apply(
            lambda row: self.calculate_baz(row[bmi_col], int(row[age_col]), str(row[sex_col])),
            axis=1,
        )
