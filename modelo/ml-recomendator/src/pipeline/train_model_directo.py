...
def entrenar_modelo_directo(
    ...
    report = classification_report(
        ...
        y_pred_val,
        target_names=[
            'DESNUT_SEV', 'DESNUT_MOD', 'RIESGO_DESN',
            'NORMAL', 'RIESGO_SOB', 'SOBREPESO', 'OBESIDAD'
        ],
        output_dict=True
    )
    ...
    if nombre_mejor == 'RandomForest':
        importance = pd.DataFrame({
            ...
            'importance': modelo_mejor.feature_importances_
...
def main():
    ...
    parser.add_argument(
        "--csv-path",
        type=Path,
        help="Ruta al CSV si data-source=csv"
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=BASE_DIR / "models" / "directo",
    ...
    feature_cols = [
        'edad_meses', 'sexo_num', 'peso_kg', 'talla_cm', 'bmi',
    ...
    # Entrenar modelo
    modelo, scaler, metricas = entrenar_modelo_directo(
        X_train, y_train,
        X_val, y_val,
        args.output
    )
    ...
    print(f"   NO necesita BAZ precalculado para hacer predicciones")
