...
def main():
    ...
    parser.add_argument(
        "--config-rf",
        type=Path,
        default=BASE_DIR / "configs/rf.yaml",
        help="Config para Random Forest",
    )
    parser.add_argument(
        "--config-nn",
        type=Path,
        default=BASE_DIR / "configs/nn.yaml",
        help="Config para Red Neuronal",
    )
    parser.add_argument(
        "--output", type=Path, default=BASE_DIR / "models", help="Directorio de salida"
    )
    parser.add_argument("--test-size", type=float, default=0.2, help="Proporción de test")
    parser.add_argument(
        "--use-cv", action="store_true", help="Usar cross-validation en lugar de train/val split"
    )
    parser.add_argument(
        "--cv-folds", type=int, default=5, help="Número de folds para cross-validation"
    )
    ...
    if args.use_cv:
        ...
        print("   que el accuracy de validación simple para datasets pequeños.")
