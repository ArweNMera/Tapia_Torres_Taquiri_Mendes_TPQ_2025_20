...
def root():
    return {
        ...
        "endpoints": {"clasificar": "/clasificar", "explicar": "/explicar", "health": "/health"},
    }
...
def clasificar(req: ClasificarRequest):
    ...
    if MODELO is None:
        raise HTTPException(
            status_code=503, detail="Modelo no disponible. Entrena primero: ./ENTRENAR_AHORA.sh"
        )
...
def modelo_info():
    ...
    if MODELO is None:
        return {"cargado": False, "mensaje": "Modelo no cargado"}
