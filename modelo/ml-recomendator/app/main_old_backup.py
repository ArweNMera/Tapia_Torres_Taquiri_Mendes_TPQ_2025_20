    ...
    for pat in patterns:
        ...
        if p.exists():
            ...
            if all(col in df.columns for col in required_cols):
                ...
                print(f"⚠️  Archivo {pat} no tiene todas las columnas requeridas. Columnas: {df.columns.tolist()}")
    if not parts:
...
def predict_ml(req: PredictMLRequest) -> PredictMLResponse:
    ...
    if DatabaseConnector is None:
        raise HTTPException(
            status_code=503,
            detail="DatabaseConnector no disponible"
        )
    ...
    try:
        ...
        if df.empty:
            raise HTTPException(
                status_code=404,
                detail=f"No se encontraron datos para nin_id={req.nin_id}"
            )
