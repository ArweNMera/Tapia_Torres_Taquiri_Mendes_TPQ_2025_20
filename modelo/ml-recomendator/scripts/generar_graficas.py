...
def grafica_1_feature_importance(model, feature_names):
    ...
    features, values = zip(*sorted_features[:10], strict=False)
...
def grafica_2_confusion_matrix(model, X, y):
    ...
    plt.title(
        "Matriz de Confusión - Random Forest (7 Categorías OMS)", fontsize=14, fontweight="bold"
    )
...
def grafica_4_arbol_decision(model):
    ...
    plot_tree(
        ...
        fontsize=7,
        max_depth=3,
    )  # Limitar profundidad para legibilidad
    plt.title(
        "Árbol de Decisión (Primer árbol del Random Forest - 7 Categorías OMS)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def grafica_8_metricas_comparacion(model, X, y):
    ...
    # Agregar valores
    for i, (p, r, f) in enumerate(zip(precision, recall, f1, strict=False)):
