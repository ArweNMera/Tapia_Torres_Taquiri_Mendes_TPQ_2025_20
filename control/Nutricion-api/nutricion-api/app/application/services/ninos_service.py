...
from app.schemas.ninos import AlergiaCreate, AnthropometryCreate, NinoCreate, NinoUpdate
...
class NinosService:
    ...
    def __init__(
        self, ninos_repository: INinosRepository, usuarios_repository: IUsuariosRepository
    ):
        self.ninos_repo = ninos_repository
        self.usuarios_repo = usuarios_repository

    def crear_nino(self, nino_data: NinoCreate, usr_id_tutor: int) -> dict[str, Any]:
        """
        ...
        if rol_codigo not in roles_validos:
            raise HTTPException(
                status_code=403, detail="El usuario no tiene permisos para crear niños"
            )
        ...
        edad_anios = (
            today.year
            - nino_data.nin_fecha_nac.year
            - (
                (today.month, today.day)
                < (nino_data.nin_fecha_nac.month, nino_data.nin_fecha_nac.day)
            )
        )
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error al crear niño: {str(e)}")

    def actualizar_nino(
        self, nin_id: int, nino_data: NinoUpdate, usr_id_solicitante: int
    ) -> dict[str, Any]:
        """
        ...
        if not self._puede_gestionar_nino(usr_id_solicitante, nino_actual.get("usr_id_tutor")):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para actualizar este niño"
            )
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error al actualizar niño: {str(e)}")

    def obtener_nino(self, nin_id: int, usr_id_solicitante: int) -> dict[str, Any]:
        """
        ...
        if not self._puede_gestionar_nino(usr_id_solicitante, nino.get("usr_id_tutor")):
            raise HTTPException(status_code=403, detail="No tienes permisos para ver este niño")
    ...
    def listar_ninos_tutor(self, usr_id_tutor: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for nino_data in ninos_data:
                ...
                antropometrias = self.ninos_repo.get_antropometrias_by_nino(
                    nino_data["nin_id"], limit=10
                )
                ...
                if antropometrias:
                    try:
                        ...
                        if estado_raw:
                            ...
                            edad_meses = nino_data.get("edad_meses") or nino_data.get(
                                "nin_edad_meses", 0
                            )
                            ...
                            ultimo_estado = {
                                ...
                                "recommendations": estado_raw.get("recomendaciones")
            ...
            raise HTTPException(status_code=500, detail=f"Error al listar niños: {str(e)}")

    def agregar_antropometria(
        self, nin_id: int, ant_data: AnthropometryCreate, usr_id_solicitante: int
    ) -> dict[str, Any]:
        """
        ...
        if not self._puede_gestionar_nino(usr_id_solicitante, nino.get("usr_id_tutor")):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para agregar datos antropométricos"
            )
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error al agregar antropometría: {str(e)}")

    def evaluar_estado_nutricional(self, nin_id: int, usr_id_solicitante: int) -> dict[str, Any]:
        """
        ...
        if not self._puede_gestionar_nino(usr_id_solicitante, nino.get("usr_id_tutor")):
            raise HTTPException(status_code=403, detail="No tienes permisos para evaluar este niño")
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error al evaluar estado nutricional: {str(e)}"
            )

    def obtener_perfil_completo(self, nin_id: int, usr_id_solicitante: int) -> dict[str, Any]:
        """
        ...
        if not self._puede_gestionar_nino(usr_id_solicitante, nino.get("usr_id_tutor")):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para ver el perfil de este niño"
            )
    ...
    def agregar_alergia(
        self, nin_id: int, alergia_data: AlergiaCreate, usr_id_solicitante: int
    ) -> dict[str, Any]:
        """
        ...
        if not self._puede_gestionar_nino(usr_id_solicitante, nino.get("usr_id_tutor")):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para agregar alergias a este niño"
            )
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="No se pudo recuperar la alergia agregada")
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error al agregar alergia: {str(e)}")

    def obtener_ultima_antropometria(
        self, nin_id: int, usr_id_solicitante: int
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error al obtener última antropometría: {str(e)}"
            )

    def obtener_alergias(self, nin_id: int, usr_id_solicitante: int) -> list[dict[str, Any]]:
        """
        ...
        if not self._puede_gestionar_nino(usr_id_solicitante, nino.get("usr_id_tutor")):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para ver las alergias de este niño"
            )

        try:
            return self.ninos_repo.obtener_alergias(nin_id)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error al obtener alergias: {str(e)}")

    def eliminar_alergia(
        self, nin_id: int, na_id: int, usr_id_solicitante: int
    ) -> list[dict[str, Any]]:
        """
        ...
        if not self._puede_gestionar_nino(usr_id_solicitante, nino.get("usr_id_tutor")):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para eliminar alergias de este niño"
            )

        try:
            return self.ninos_repo.eliminar_alergia(na_id, nin_id)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error al eliminar alergia: {str(e)}")
    ...
    def buscar_tipos_alergias(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error al buscar tipos de alergias: {str(e)}"
            )

    def asignar_tutor(
        self, nin_id: int, nuevo_tutor_id: int, usr_id_solicitante: int
    ) -> dict[str, Any]:
        """
        ...
        if not (es_admin or es_tutor_actual):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para cambiar el tutor de este niño"
            )
        ...
        if rol_codigo not in roles_validos:
            raise HTTPException(
                status_code=403, detail="El usuario no tiene un rol válido para ser tutor"
            )

        try:
            return self.ninos_repo.asignar_tutor(nin_id, nuevo_tutor_id)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error al asignar tutor: {str(e)}")

    def obtener_nino_completo(self, nin_id: int, usr_id_solicitante: int) -> dict[str, Any]:
        """
        ...
        if estado_nutricional_raw:
            ...
            estado_nutricional = {
                ...
                "recommendations": estado_nutricional_raw.get("recomendaciones")
                or generar_recomendaciones_nutricionales(clasificacion, imc, edad_meses),
    ...
    def obtener_historial_antropometrico(
        self, nin_id: int, usr_id_solicitante: int, limit: int = 10
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error al obtener historial antropométrico: {str(e)}"
            )

    def obtener_alergias_nino(self, nin_id: int, usr_id_solicitante: int) -> list[dict[str, Any]]:
        """
    ...
    def _puede_gestionar_nino(self, usr_id: int, tutor_id: int | None) -> bool:
        """
