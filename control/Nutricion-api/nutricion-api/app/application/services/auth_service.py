...
from app.core.firebase import FirebaseNotConfigured, verify_firebase_token
...
class AuthService:
    ...
    def login_user(self, user_login: UserLogin) -> Token:
        ...
        access_token = self.jwt_service.create_access_token(data={"sub": user.usr_usuario})
    ...
    def login_with_firebase(self, id_token_value: str) -> Token:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Firebase no está configurado en el backend"
            ) from exc
        except (firebase_exceptions.FirebaseError, ValueError) as exc:
            raise HTTPException(status_code=401, detail="Token de Firebase inválido") from exc
        ...
        if id_info.get("email_verified") is False and provider_id not in {
            "github.com",
            "facebook.com",
            "microsoft.com",
        }:
    ...
    def complete_google_oauth(self, code: str) -> tuple[Token, str | None]:
        """
    ...
    def _finalize_google_login(self, id_info: dict) -> Token:
        ...
        if not user:
            raise HTTPException(
                status_code=500, detail="No se pudo completar el inicio de sesión con Google"
            )
    ...
    def _register_google_user(self, id_info: dict) -> UserResponse | None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="No se pudo registrar el usuario de Google"
            ) from exc

        return self.repository.get_user_by_email(id_info.get("email"))

    def _update_user_avatar_if_needed(
        self, user: UserResponse, picture_url: str, id_info: dict, created_new_user: bool
    ):
...
def get_current_user(
    ...
    if not authorization or not authorization.lower().startswith("bearer "):
        raise HTTPException(
            status_code=401, detail="Token de autorización requerido para el sistema"
        )
...
def complete_google_oauth(db: Session, code: str) -> tuple[Token, str | None]:
    """Función legacy"""
    auth_service = AuthService(
        repository=UsuariosRepository(db),
        password_service=PasswordService(),
