...
class GoogleOAuthClient:
    ...
    def verify_id_token(self, id_token_value: str) -> dict[str, any]:
        ...
        if not self.client_id:
            raise HTTPException(
                status_code=500, detail="Google Sign-In no está configurado en el backend"
            )
    ...
    def exchange_code_for_tokens(self, code: str) -> dict[str, str]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Error al obtener tokens de Google: {str(exc)}"
            ) from exc
    ...
    def get_user_info(self, access_token: str) -> dict[str, any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Error al obtener info de Google: {str(exc)}"
            ) from exc
    ...
    def resolve_redirect_from_state(self, state: str) -> str:
        ...
        try:
            ...
            if not redirect_target:
                raise HTTPException(
                    status_code=400, detail="Estado de OAuth inválido: falta redirect_to"
                )
            ...
            raise HTTPException(
                status_code=400, detail="Estado de OAuth inválido o expirado"
            ) from exc

    def build_redirect_url(
        self, target: str, token: str = None, avatar_url: str = None, error: str = None
    ) -> str:
        """
