...
def get_or_create_self_child(
    ...
    if not perfil or not perfil.get("fecha_nac") or not perfil.get("genero"):
        raise HTTPException(
            status_code=400,
            detail="Completa tu fecha de nacimiento y género en el perfil antes de registrar medidas",
        )
    genero = perfil.get("genero")
    if genero not in ("M", "F"):
        raise HTTPException(
            status_code=400,
            detail="El género debe ser M o F para crear el perfil antropométrico personal",
        )
    ...
    try:
        ...
        for cand in posibles:
            if (
                cand.get("nin_fecha_nac") == perfil.get("fecha_nac")
                and cand.get("nin_sexo") == genero
            ):
    ...
    nombres = (
        (perfil.get("usr_nombre") or current_user.usr_nombre)
        if hasattr(current_user, "usr_nombre")
        else perfil.get("usr_nombre")
    )
    apellidos = (
        (perfil.get("usr_apellido") or current_user.usr_apellido)
        if hasattr(current_user, "usr_apellido")
        else perfil.get("usr_apellido")
    )
    display_name = (
        f"{nombres} {apellidos}" if nombres or apellidos else current_user.usr_usuario
    ).strip()
    ...
    if not created:
        raise HTTPException(
            status_code=400, detail="No se pudo crear el perfil antropométrico personal"
        )
    return NinoResponse(**created)


@router.post(
    "/profiles", response_model=CreateChildProfileResponse, status_code=status.HTTP_201_CREATED
)
...
@router.post(
    "/{nin_id}/anthropometry",
    response_model=AnthropometryResponse,
...
def assign_child_tutor(
    ...
    is_current_responsible = (
        nino_dict.get("usr_id_tutor") == current_user.usr_id
        or nino_dict.get("usr_id_propietario") == current_user.usr_id
    )
...
def create_child_basic(
    ...
    # 🔍 DEBUG LOG
    logger.warning("🔍 CREATE CHILD DEBUG:")
    logger.warning("  📝 Datos recibidos del formulario:")
    ...
    logger.warning("  👤 Usuario actual (tutor/propietario):")
    logger.warning(f"    - usr_id: {current_user.usr_id}")
    logger.warning(
        f"    - usr_usuario: {current_user.usr_usuario if hasattr(current_user, 'usr_usuario') else 'N/A'}"
    )
...
@router.post(
    "/{nin_id}/alergias", response_model=AlergiaResponse, status_code=status.HTTP_201_CREATED
)
