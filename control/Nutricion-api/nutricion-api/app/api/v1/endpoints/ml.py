...
class AnalisisNutricionalRequest(BaseModel):
    ...
    peso_kg: Optional[float] = Field(None, description="Peso en kg (opcional)")
    talla_cm: Optional[float] = Field(None, description="Talla en cm (opcional)")
...
class AnalisisNutricionalResponse(BaseModel):
    ...
    imc: Optional[float]
    diagnostico: str
    z_score_imc: Optional[float]
    percentil: Optional[float]
    nivel_riesgo: str
    riesgo_porcentaje: Optional[float]
...
async def analisis_nutricional(
    ...
    if req.peso_kg is not None and req.talla_cm is not None:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"No se pudo registrar la antropometría: {exc}")
    ...
    try:
        ...
        if not estado:
            raise HTTPException(
                status_code=400,
                detail="No fue posible obtener la evaluación nutricional"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error en análisis nutricional: {exc}"
        )
